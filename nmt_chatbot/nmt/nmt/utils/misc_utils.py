...
def check_tensorflow_version():
  ...
  for min, cur in zip(min_tf_version, current_tf_version):
    if cur < min:
      raise EnvironmentError("Tensorflow version must >= %s" % min_tf_version)

  # if tf.__version__ < min_tf_version:                                         # original code checked for strings, so 1.14.0 was seen as < than 1.4.0
  #   raise EnvironmentError("Tensorflow version must >= %s" % min_tf_version)
